...
```
NhtsaService → VehicleAssembler → VinLookupService → VehiclePresenter
...
- **`VehicleData`** / **`PowertrainData`** — value object structs in `app/services/`; `PowertrainData` type is one of `:gas`, `:mild_hybrid`, `:plugin_hybrid`, `:ev`
...
- NHTSA response fixtures for gas, EV, and PHEV vehicles live in `spec/fixtures/`. Use these for `VehicleAssembler` tests.
